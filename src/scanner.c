...
enum TokenType { NEWLINE, WHITESPACE, LOGICAL_LINE_WHITESPACE, BODY_SEPARATOR };
...
bool tree_sitter_test_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[NEWLINE] || valid_symbols[WHITESPACE] ||
      valid_symbols[LOGICAL_LINE_WHITESPACE] || valid_symbols[BODY_SEPARATOR]) {
    ...
    if (valid_symbols[NEWLINE] || valid_symbols[LOGICAL_LINE_WHITESPACE] ||
      ...
      if (at_newline_with_optional_linefeed(lexer)) {

        DEBUGPRINT("Found double newline\n");
        advance(lexer);
        mark_end(lexer);
...
void tree_sitter_test_external_scanner_destroy(void *payload) { return; }

unsigned tree_sitter_test_external_scanner_serialize(void *payload,
                                                     char *buffer) {
  return 0;
}

void tree_sitter_test_external_scanner_deserialize(void *payload,
                                                   const char *buffer,
                                                   unsigned length) {
  return;
}
